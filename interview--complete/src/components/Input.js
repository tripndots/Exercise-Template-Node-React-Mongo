...
export default (props) => {
  function handleChange (e) {
    ...
    if (e.target.value !== '') {
      if (!(placeholder.classList.contains('input__placeholder--datain'))) { placeholder.classList.add('input__placeholder--datain') }
    } else {
  ...
  switch (props.type) {
    case 'select':
      Input = <select defaultValue='default' name={props.name} onChange={(e) => handleChange(e)} className='input'>
        <option value='default' disabled hidden>Select</option>
        { props.options ? props.options.map((option, i) => <option name={option} key={i}>{ option }</option>) : null }
      </select>
      break
    case 'multiple':
      Input = <select name={props.name} onChange={(e) => handleChange(e)} multiple className='inputLG'>
        { props.options ? props.options.map((option, i) => <option name={option} key={i}>{ option }</option>) : null }
      </select>
      break
    case 'area':
